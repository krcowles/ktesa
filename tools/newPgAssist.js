	...
	function bldRow(noOfPix, rowHt, isLast) {
	    ...
	    if ( isLast == true ) { // 0 to n images + iframe
	        if ( noOfPix == 0 ) {  // iframe only - no images
                msg = '<p>Input rowHt = ' + rowHt + '</p>';
                $('#tmp_dump_area').append(msg);
                pic += '\n\t<scan><iframe id="theMap" height="' + rowHt +
                       '" width="' + rowHt + '" src="' + geoMap +
                       '"></iframe></scan>\n</div>';
	        } else {  // images + iframe
    	        for ( var q=0; q<noOfPix; q++ ) {
                    // place image(s)
                    msg = '<p>[HB] LAST: FRAME w/PIX; q = ' + q + '; noOfPix = ' + noOfPix + '</p>';
                    $('#tmp_dump_area').append(msg);
                    pic += '\n\t<img id="pic' + picId + '" height="' + rowHt + '" src="' + nSize[picId] + '" alt="' + picDescs[picId] + '" />';
	        ...
	        for ( var r=0; r<noOfPix; r++ ) {
	            pic += '\n\t<img id="pic' + picId + '" height="' + rowHt + 
	                '" src="' + nSize[picId] + '" alt="' + picDescs[picId] + '" />';
	                cap += '\n\t\t<li>' + captions[picId] + '</li>';
                    lnk += '\n\t\t<li>' + albPics[picId] + '</li>';
                    picId++;
	        }
