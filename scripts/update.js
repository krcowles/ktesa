...
let sendEmail = document.getElementById('adminmail');
sendEmail.textContent = adminEmail;
let sendEmsg = document.getElementById('adminmsg');
var update_status = '';
        ...
        registration.unregister().then(async (success) => {
            if (success) {
                ...
                if (update_status == '') {
                    ...
                    if (update_status == '') {
                        $.post('updateDbVersion.php', { user: user, latest: latest_ver }, (result) => {
                            if (result !== 'ok') {
