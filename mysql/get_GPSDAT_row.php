...
if (mysqli_num_rows($result) !== 0) {
    $propHtml = '<p id="proptitle"  style="position:relative;top:-12px;">- Proposed Hike Data</p> ' . "\n" .
                    '<ul id="plinks">' . PHP_EOL;
    $actHtml = '<p id="acttitle"  style="position:relative;top:-12px;">- Actual Hike Data</p>' . "\n" .
                    '<ul id="alinks">' . PHP_EOL;
    $pgpxHtml = '';
    $pothrHtml = '';
    $agpxHtml = '';
    $aothrHtml = '';
    while ($row = mysqli_fetch_assoc($result)) {
        if ($row['datType'] === 'P') {
            ...
            if ($ext === 'gpx') {
                ...
                $pgpxHtml .= '<li class="gpslnks">' . $row['label'] . '&nbsp;' . $row['clickText'] .
