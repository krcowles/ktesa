...
<body>
	<div class="container_16 clearfix">
        ...
        <?php
			...
			/*    PROCESS IMAGE ROWS    */
			# tmp alt description array:
			$caption = array("a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p");
			$picNo = 0;
        	for ($k=0; $k<$rowCount; $k++) {
				$rowParms = explode("^",$rows[$k]);
				$noOfPix = intval($rowParms[0]);
				$rowHt = $rowParms[1];
				$rowHtml = '<div id="row' . $k . '" class="ImgRow">';
				for ($m=0; $m<$noOfPix; $m++) {
					$pbase = 2 * $m + 2;   //base pointer to picture elements in $rowParms
					if ($m === 0) {
						$style = '';
					} else {
					...
					# echo "pbase is " . $pbase;
					$rowHtml = $rowHtml . '<img id="pic' . $picNo . '" style="' . $style . '" width="' .
						$rowParms[$pbase] . '" height="' . $rowHt . '" src="' . $rowParms[$pbase+1] .
						'" alt="' . $caption[$picNo] . '" />';
					$picNo++;
				}
				$rowHtml = $rowHtml . '</div>';
...
#    no:3 ht:218 0w:316 0src:http//kkj 0cap:yyyy 1w:316 1src:http://sjkfjd 1cap:xxx 2w:316 
